...
def issaugoti_profilio_paveiksleli(failas, vartotojas):
    if failas and leistinas_failas(failas.filename):
        ...
        data_str = datetime.utcnow().strftime('%Y%m%d%H%M%S')
        failo_pavadinimas = secure_filename(f"{vartotojas.vardas}_{vartotojas.pavarde}_{data_str}.{pratesimas}")
